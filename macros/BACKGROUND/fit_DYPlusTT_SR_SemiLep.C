void fit_DYPlusTT_SR_SemiLep(){
  ...
  for(int i=0; i<4; i++){
    ...
    BackgroundEstimation("MuoTau", XMassWidth, XMassBin, XMassMin, XMassMax, histo_PRE1, histo_MC1, BkgYield);
    Fitting(c1, MAX, histo_PRE1, histo_MC1, FUNCTION, XMassWidth, XMassBin, XMassMin, XMassMax, "MuoTau", function, SignalMass, BkgYield, save, pull, resid);
    		
    TCanvas* c2 = new TCanvas("c2","c2",0,0,600,600); 
    BackgroundEstimation("EleTau", XMassWidth, XMassBin, XMassMin, XMassMax, histo_PRE2, histo_MC2, BkgYield);
    Fitting(c2, MAX, histo_PRE2, histo_MC2, FUNCTION, XMassWidth, XMassBin, XMassMin, XMassMax, "EleTau", function, SignalMass, BkgYield, save, pull, resid);
...
void Fitting(TCanvas* c, float MAX, TH1F *histo_PRE, TH1F *histo_MC, TString FUNCTION, float XMassWidth, float XMassBin,
  ...
  if(SignalMass==1000)      {
    TFile *fileSig = TFile::Open("../RISULTATI/LepIso_010413/ZH1000_SL.root");  
    ...
    TFile *fileSig = TFile::Open("../RISULTATI/LepIso_010413/ZH1500_SL.root");  
    TreeSig=(TTree*)fileSig->Get(openTree); 
    ...
    TFile *fileSig = TFile::Open("../RISULTATI/LepIso_010413/ZH2000_SL.root");  
    TreeSig=(TTree*)fileSig->Get(openTree); 
    ...
    TFile *fileSig = TFile::Open("../RISULTATI/LepIso_010413/ZH2500_SL.root");  
    TreeSig=(TTree*)fileSig->Get(openTree); 
  ...
  TTree *SignalTree = TreeSig->CopyTree("met>100 && lepCorrPFIso<0.1 && uncorrmet>30 && trigger==1");
  ...
  SignalTree->Draw("XMassSVFit>>data_sig(15,0,3000)","PUWeight*(met>100 && lepCorrPFIso<0.1)");
  ...
  TFile *fileDat = TFile::Open("../RISULTATI/LepIso_010413/data_SL.root"); 
...
void SaveWorkspace(TTree *Tree1, RooFFTConvPdf background, RooAddPdf signal, char *channel, char *function, int SignalMass, float BkgYield, float SigYield,
  ...
  TTree *SelectedTree = Tree1->CopyTree("met>100 && lepCorrPFIso<0.1 && uncorrmet>30 && trigger==1");
  RooRealVar XMassSVFit("XMassSVFit","XMassSVFit",0,MAX);
...
void BackgroundEstimation(char *channel, float XMassWidth, float XMassBin, float XMassMin, float XMassMax, TH1F *histo_PRE, TH1F *histo_MC, float & BkgYield){
  ...
  for(int i=0; i<XMassBin; i++){
    ...
    if(!lepTauIso){
      ...
      else                                      sprintf(CUT, "trigger*PUWeight*(met>100 && uncorrmet>30 && XMassSVFit>%f && XMassSVFit<%f)",XMassMin,XMassMax);
      ...
      else                                      sprintf(CUT_SR,"trigger*PUWeight*(met>100 && uncorrmet>30 && XMassSVFit>%f && XMassSVFit<%f)",XMassMin,XMassMax);
    } else {
      if(CHANNEL=="EleEle" || CHANNEL=="MuoMuo")sprintf(CUT, "trigger*PUWeight*(met>100 && uncorrmet>30 && XMassSVFit>%f && XMassSVFit<%f && MassVis>10)",XMassMin,XMassMax);
      else if(CHANNEL=="EleMuo") 	        sprintf(CUT, "trigger*PUWeight*(met>100 && uncorrmet>30 && XMassSVFit>%f && XMassSVFit<%f)",XMassMin,XMassMax);
      else if(CHANNEL=="EleTau")  	        sprintf(CUT, "trigger*PUWeight*(met>100 && uncorrmet>30 && XMassSVFit>%f && XMassSVFit<%f && lepCorrPFIso<0.1)",
							XMassMin,XMassMax);
      else                                      sprintf(CUT, "trigger*PUWeight*(met>100 && uncorrmet>30 && XMassSVFit>%f && XMassSVFit<%f && lepCorrPFIso<0.2)",
							XMassMin,XMassMax);
      if(CHANNEL=="EleEle" || CHANNEL=="MuoMuo")sprintf(CUT_SR,"trigger*PUWeight*(met>100 && uncorrmet>30 && XMassSVFit>%f && XMassSVFit<%f && MassVis>10)",XMassMin,XMassMax);
      else if(CHANNEL=="EleMuo")                sprintf(CUT_SR,"trigger*PUWeight*(met>100 && uncorrmet>30 && XMassSVFit>%f && XMassSVFit<%f)",XMassMin,XMassMax);
      else if(CHANNEL=="EleTau")  	        sprintf(CUT_SR,"trigger*PUWeight*(met>100 && uncorrmet>30 && XMassSVFit>%f && XMassSVFit<%f && lepCorrPFIso<0.1)",
							XMassMin,XMassMax);
      else   	                                sprintf(CUT_SR,"trigger*PUWeight*(met>100 && uncorrmet>30 && XMassSVFit>%f && XMassSVFit<%f && lepCorrPFIso<0.2)",
							XMassMin,XMassMax);
    }
