{
  ...
  if(CHANNEL=="MuoMuo") hs->SetMaximum(50);
  if(CHANNEL=="EleEle") hs->SetMaximum(30);
  if(CHANNEL=="EleMuo") hs->SetMaximum(40);
  if(CHANNEL=="EleTau") hs->SetMaximum(40);
  if(CHANNEL=="MuoTau") hs->SetMaximum(70);
  hs->GetXaxis()->SetNdivisions(509);
  hs->SetMinimum(0);

  //histo_DrY->Draw("histo");
  histo_PRE->Draw("E same");
  histo_OBS->Draw("E same");
  ...
  TLegend *pl2 = new TLegend(0.45,0.55,0.89,0.89);
  ...
  if(save){
    c1->SaveAs("DYPlusTT_"+CHANNEL+".png");
    //c1->SaveAs("DYPlusTT_"+CHANNEL+".root");
    //c1->SaveAs("DYPlusTT_"+CHANNEL+".C");
  }
