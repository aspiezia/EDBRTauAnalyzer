{
  ...
  //PLOT.push_back("MassVis");        BIN.push_back(30); MIN.push_back(0);   MAX.push_back(300);  MAXY.push_back(10);   AXIS.push_back("M_{vis}(lep1, lep2) [GeV]");
  //PLOT.push_back("MassEff");        BIN.push_back(30); MIN.push_back(0);   MAX.push_back(300);  MAXY.push_back(10);   AXIS.push_back("M_{eff}(lep1, lep2) [GeV]");
  PLOT.push_back("MassCA");        BIN.push_back(30); MIN.push_back(0);   MAX.push_back(300);  MAXY.push_back(10);   AXIS.push_back("M_{CA}(lep1, lep2) [GeV]");
  //PLOT.push_back("MassSvfit");        BIN.push_back(30); MIN.push_back(0);   MAX.push_back(300);  MAXY.push_back(10);   AXIS.push_back("M_{svfit}(lep1, lep2) [GeV]");
  ...
  for(int i=0; i<1; i++){
    ...
    for(int j=0; j<3; j++){
      char CUT [500]; 
      char openTree[500];
      if(j==0)      { sprintf(openTree, "demo/TreeEleMuo"); sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>30)");}
      else if(j==1) { sprintf(openTree, "demo/TreeMuoMuo"); sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>30 && MassVis>10)");}
      else if(j==2) { sprintf(openTree, "demo/TreeEleEle"); sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>30 && MassVis>10)");}
    ...
    if(save) c1->SaveAs(name+"_FullyleptonChannels_SR.png");
  }
}
