...
void limitAllChan(bool only_leptonTAG=false, bool save_plots = false, float min=0.006, float max=900., TString plot_name="Limit_prova.pdf") {
  setTDRStyle();
  TCanvas* c1  = new TCanvas("c1","c1",0,0,800,600); //c1->SetLogy();
  c1->SetLogy();
  save_plots = true;
  plot_name="LimitAllChannel.pdf";

  Double_t XMass[val]    = {1000, 1500, 2000, 2500};
	
  //LIMIT - Breit-Wigner
  Double_t Limit_Obs[val]     = {0,0,0,0};
  Double_t Limit_Exp_m2s[val] = {1.23, 0.59, 0.18, 0.20};
  Double_t Limit_Exp_m1s[val] = {1.69, 0.82, 0.30, 0.34};
  Double_t Limit_Exp[val]     = {2.45, 1.23, 0.53, 0.59};
  Double_t Limit_Exp_p1s[val] = {3.66, 1.88, 0.95, 1.05};
  Double_t Limit_Exp_p2s[val] = {5.30, 2.79, 1.59, 1.76};
