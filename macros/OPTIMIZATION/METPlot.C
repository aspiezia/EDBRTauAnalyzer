{
  ...
  PLOT.push_back("met");            BIN.push_back(50); MIN.push_back(0);   MAX.push_back(1000); MAXY.push_back(2);   AXIS.push_back("met [GeV]");
  ...
  for(int type=0; type<2; type++){
	...
	if(j==0){ 
	  ...
	  sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && met>%f)",CUTPre,METCutFL,MassVisCutFL1,lep1PtCut,deltaRCutFL,MET);
	}else if(j==1){ 
	  sprintf(openTree, "demo/TreeMuoMuo"); 
	  sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && met>%f)",CUTPre,METCutFL,MassVisCutFL2,lep1PtCut,deltaRCutFL,MET);
	}else if(j==2){ 
	  sprintf(openTree, "demo/TreeEleEle"); 
	  sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && met>%f)",CUTPre,METCutFL,MassVisCutFL2,lep1PtCut,deltaRCutFL,MET);
	}else if(j==3){ 
	  sprintf(openTree, "demo/TreeMuoTau");
	  sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && met>%f)",CUTPre,METCutSL,MassVisCutSL, tauPtCut, deltaRCutSL,MET);
	}else if(j==4){ 
	...
	c1->SetLogy();
      ...
      if(save && i==0 && !SL) 
	c1->SaveAs("MET_MET"+met+"_lep1Pt"+lep1pt+"_tauPt"+taupt+"_btag"+btag+"_dR"+drcut+"_massSvfit"+masssvfitcut+"_massVis"+massviscut+"_FL1000.pdf");
      if(save && i==0 && SL)  
	c1->SaveAs("MET_MET"+met+"_lep1Pt"+lep1pt+"_tauPt"+taupt+"_btag"+btag+"_dR"+drcut+"_massSvfit"+masssvfitcut+"_massVis"+massviscut+"_SL1000.pdf");
    }
  }
}
