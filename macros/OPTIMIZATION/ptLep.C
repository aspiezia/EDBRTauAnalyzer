{
  ...
  bool save=true; 
  bool log=false;
  float lep1PtCut=10; float tauPtCut=20; int bTagCut=1; float deltaRCutFL=1; float deltaRCutSL=1; float MassSvfitCut=10000; float METCutFL = 100; float METCutSL = 50; 
  ...
  PLOT.push_back("lep1Pt");     BIN.push_back(50); MIN.push_back(0);   MAX.push_back(500);   MAXY.push_back(40);   AXIS.push_back("#tau pt [GeV]");
  PLOT.push_back("lep2Pt");     BIN.push_back(50); MIN.push_back(0);   MAX.push_back(500);   MAXY.push_back(20);   AXIS.push_back("lep pt [GeV]");
  PLOT.push_back("lep1Pt");     BIN.push_back(50); MIN.push_back(0);   MAX.push_back(500);   MAXY.push_back(8);    AXIS.push_back("lead lep pt [GeV]");
  PLOT.push_back("lep2Pt");     BIN.push_back(50); MIN.push_back(0);   MAX.push_back(500);   MAXY.push_back(12);   AXIS.push_back("sublead lep pt [GeV]");
  
  TH1F *ptLepoptimization1  = new TH1F("ptLepoptimization1","ptLepoptimization1",15,7.5, 82.5);
  TH1F *ptLepoptimization2  = new TH1F("ptLepoptimization2","ptLepoptimization2",15,7.5, 82.5);
  TH1F *SignalEfficiency1   = new TH1F("SignalEfficiency1", "SignalEfficiency1", 15,7.5, 82.5);
  TH1F *SignalEfficiency2   = new TH1F("SignalEfficiency2", "SignalEfficiency2", 15,7.5, 82.5);
  TH1F *BackgroundYield1    = new TH1F("BackgroundYield1",  "BackgroundYield1",  15,7.5, 82.5);
  TH1F *BackgroundYield2    = new TH1F("BackgroundYield2",  "BackgroundYield2",  15,7.5, 82.5);
  ...
  for(int type=0; type<2; type++){
    ...
    if(type==0) SL=true;
    if(type==1) SL=false;
    for(int PT=0; PT<PLOT.size();PT++){
      if(SL){  if(!(PT==0 || PT==1)) continue;}
      if(!SL){ if(!(PT==2 || PT==3)) continue;}
      char *plot = PLOT[PT].c_str();
      TString name = PLOT[PT];
      int bin=BIN[PT]; 
      float min=MIN[PT]; 
      float max=MAX[PT]; 
      float maxy=MAXY[PT];
      TString axis = AXIS[PT];
      for(int k=0; k<2; k++){
	...
	for(int i=0; i<16; i++){
	  ...
	  for(int j=jMIN; j<jMAX; j++){
		...
		sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && XMassSVFit>837  && XMassSVFit<1163 && %s>%f)",
			CUTPre,METCutFL,MassVisCutFL1,lep1PtCut,deltaRCutFL,plot,ptLep);
	      }else if(j==1){ 
		sprintf(openTree, "demo/TreeMuoMuo");
		sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && XMassSVFit>837  && XMassSVFit<1163 && %s>%f)",
			CUTPre,METCutFL,MassVisCutFL2,lep1PtCut,deltaRCutFL,plot,ptLep);
	      }else if(j==2){ 
		...
		sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && XMassSVFit>837  && XMassSVFit<1163 && %s>%f)",
			CUTPre,METCutSL,MassVisCutSL, tauPtCut, deltaRCutSL,plot,ptLep);
	      }else if(j==4){ 
		...
		sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && XMassSVFit>837  && XMassSVFit<1163 && %s>%f)",
	      ...
	      if(j==0){ 
		sprintf(openTree, "demo/TreeEleMuo"); 
		sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && XMassSVFit>2125 && XMassSVFit<2875 && %s>%f)",
			CUTPre,METCutFL,MassVisCutFL1,lep1PtCut,deltaRCutFL,plot,ptLep);
	      }else if(j==1){ 
		sprintf(openTree, "demo/TreeMuoMuo"); 
		sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && XMassSVFit>2125 && XMassSVFit<2875 && %s>%f)",
      ...
      TString LABEL = "";
      if(PT==0)      LABEL = "#tau pt threshold";
      else if(PT==1) LABEL = "lep pt threshold";
      else if(PT==2) LABEL = "lead lep pt threshold";
      else if(PT==3) LABEL = "sublead lep pt threshold";

    
      TCanvas* c2 = new TCanvas("c2","c2",0,0,800,600);
      ptLepoptimization1->Draw("E");
      ptLepoptimization1->SetMinimum(0.0);
      ptLepoptimization1->SetMaximum(0.4);
      ptLepoptimization1->SetMarkerStyle(21);
      ptLepoptimization1->SetLineColor(1);
      ptLepoptimization1->GetYaxis()->SetTitleSize(0.045);
      ptLepoptimization1->GetXaxis()->SetTitleSize(0.045);
      ptLepoptimization1->GetYaxis()->SetLabelSize(0.045);
      ptLepoptimization1->GetXaxis()->SetLabelSize(0.045);
      ptLepoptimization1->SetTitle("");
      ptLepoptimization1->GetXaxis()->SetTitle(LABEL);
      ptLepoptimization1->GetYaxis()->SetTitle("#epsilon_{S}/1+#sqrt{B}");
      if(PT==0 && SL) ptLepoptimization1->GetXaxis()->SetRangeUser(20,SignalEfficiency1->GetXaxis()->GetXmax());
      else            ptLepoptimization1->GetXaxis()->SetRangeUser(10,SignalEfficiency1->GetXaxis()->GetXmax());
      if(save && !SL) 
	c2->SaveAs(name+"_1000_OPTIMIZATION_MET"+met+"_lep1Pt"+lep1pt+"_tauPt"+taupt+"_btag"+btag+"_dR"+drcut+"_massSvfit"+masssvfitcut+"_massVis"+massviscut+"_FL.pdf");
      if(save && SL)  
	c2->SaveAs(name+"_1000_OPTIMIZATION_MET"+met+"_lep1Pt"+lep1pt+"_tauPt"+taupt+"_btag"+btag+"_dR"+drcut+"_massSvfit"+masssvfitcut+"_massVis"+massviscut+"_SL.pdf");
    
      TCanvas* c3 = new TCanvas("c3","c3",0,0,800,600);
      ptLepoptimization2->Draw("E");
      ptLepoptimization2->SetMinimum(0.0);
      ptLepoptimization2->SetMaximum(0.8);
      ptLepoptimization2->SetMarkerStyle(21);
      ptLepoptimization2->SetLineColor(1);
      ptLepoptimization2->GetYaxis()->SetTitleSize(0.045);
      ptLepoptimization2->GetXaxis()->SetTitleSize(0.045);
      ptLepoptimization2->GetYaxis()->SetLabelSize(0.045);
      ptLepoptimization2->GetXaxis()->SetLabelSize(0.045);
      ptLepoptimization2->SetTitle("");
      ptLepoptimization2->GetXaxis()->SetTitle(LABEL);
      ptLepoptimization2->GetYaxis()->SetTitle("#epsilon_{S}/1+#sqrt{B}");
      if(PT==0 && SL) ptLepoptimization2->GetXaxis()->SetRangeUser(20,SignalEfficiency1->GetXaxis()->GetXmax());
      else            ptLepoptimization2->GetXaxis()->SetRangeUser(10,SignalEfficiency1->GetXaxis()->GetXmax());
      if(save && !SL) 
	c3->SaveAs(name+"_2500_OPTIMIZATION_MET"+met+"_lep1Pt"+lep1pt+"_tauPt"+taupt+"_btag"+btag+"_dR"+drcut+"_massSvfit"+masssvfitcut+"_massVis"+massviscut+"_FL.pdf");
      if(save && SL)  
	c3->SaveAs(name+"_2500_OPTIMIZATION_MET"+met+"_lep1Pt"+lep1pt+"_tauPt"+taupt+"_btag"+btag+"_dR"+drcut+"_massSvfit"+masssvfitcut+"_massVis"+massviscut+"_SL.pdf");
      ...
      SignalEfficiency1->GetXaxis()->SetTitle(LABEL);
      SignalEfficiency1->GetYaxis()->SetTitle("Signal Efficiency");
      if(PT==0 && SL) SignalEfficiency1->GetXaxis()->SetRangeUser(20,SignalEfficiency1->GetXaxis()->GetXmax());
      else            SignalEfficiency1->GetXaxis()->SetRangeUser(10,SignalEfficiency1->GetXaxis()->GetXmax());
      ...
      Double_t xmin = 0;
      if(!(PT==0 && SL)) xmin = 10-2.5;
      if(PT==0 && SL)    xmin = 20-2.5;
      ...
      BackgroundYield1->GetXaxis()->SetTitle(LABEL);
      ...
      if(PT==0 && SL) BackgroundYield1->GetXaxis()->SetRangeUser(20,SignalEfficiency1->GetXaxis()->GetXmax());
      else            BackgroundYield1->GetXaxis()->SetRangeUser(10,SignalEfficiency1->GetXaxis()->GetXmax());
      ...
      if(save && !SL) 
	c4->SaveAs(name+"_1000_MET"+met+"_lep1Pt"+lep1pt+"_tauPt"+taupt+"_btag"+btag+"_dR"+drcut+"_massSvfit"+masssvfitcut+"_massVis"+massviscut+"_FL.pdf");
      if(save && SL)  
	c4->SaveAs(name+"_1000_MET"+met+"_lep1Pt"+lep1pt+"_tauPt"+taupt+"_btag"+btag+"_dR"+drcut+"_massSvfit"+masssvfitcut+"_massVis"+massviscut+"_SL.pdf");
      ...
      SignalEfficiency2->GetXaxis()->SetTitle(LABEL);
      SignalEfficiency2->GetYaxis()->SetTitle("Signal Efficiency");
      if(PT==0 && SL) SignalEfficiency2->GetXaxis()->SetRangeUser(20,SignalEfficiency1->GetXaxis()->GetXmax());
      else            SignalEfficiency2->GetXaxis()->SetRangeUser(10,SignalEfficiency1->GetXaxis()->GetXmax());
      ...
      BackgroundYield2->GetXaxis()->SetTitle(LABEL);
      ...
      if(PT==0 && SL) BackgroundYield2->GetXaxis()->SetRangeUser(20,SignalEfficiency1->GetXaxis()->GetXmax());
      else            BackgroundYield2->GetXaxis()->SetRangeUser(10,SignalEfficiency1->GetXaxis()->GetXmax());
      ...
      if(save && !SL) 
	c5->SaveAs(name+"_2500_MET"+met+"_lep1Pt"+lep1pt+"_tauPt"+taupt+"_btag"+btag+"_dR"+drcut+"_massSvfit"+masssvfitcut+"_massVis"+massviscut+"_FL.pdf");
      if(save && SL)  
	c5->SaveAs(name+"_2500_MET"+met+"_lep1Pt"+lep1pt+"_tauPt"+taupt+"_btag"+btag+"_dR"+drcut+"_massSvfit"+masssvfitcut+"_massVis"+massviscut+"_SL.pdf");
    }
  }
}
