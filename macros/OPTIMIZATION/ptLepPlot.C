{
  ...
  PLOT.push_back("lep1Pt");     BIN.push_back(50); MIN.push_back(0);   MAX.push_back(500);   MAXY.push_back(5);   AXIS.push_back("#tau pt [GeV]");
  PLOT.push_back("lep2Pt");     BIN.push_back(50); MIN.push_back(0);   MAX.push_back(500);   MAXY.push_back(20);   AXIS.push_back("lep pt [GeV]");
  PLOT.push_back("lep1Pt");     BIN.push_back(50); MIN.push_back(0);   MAX.push_back(500);   MAXY.push_back(1);    AXIS.push_back("lead lep pt [GeV]");
  ...
  for(int type=0; type<2; type++){
	...
	for(int j=jMIN; j<jMAX; j++){
	  ...
	  if(j==0){ 
	    ...
	    sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && %s>%f)",CUTPre,METCutFL,MassVisCutFL1,lep1PtCut,deltaRCutFL,plot,ptLep);
	  }else if(j==1){ 
	    sprintf(openTree, "demo/TreeMuoMuo");
	    sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && %s>%f)",CUTPre,METCutFL,MassVisCutFL2,lep1PtCut,deltaRCutFL,plot,ptLep);
	  }else if(j==2){ 
	    sprintf(openTree, "demo/TreeEleEle"); 
	    sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && %s>%f)",CUTPre,METCutFL,MassVisCutFL2,lep1PtCut,deltaRCutFL,plot,ptLep);
	  }else if(j==3){ 
	    sprintf(openTree, "demo/TreeMuoTau");
	    sprintf(CUT, "%s && met>%f && MassVis>%f && lep1Pt>%f && dRLep1Lep2<%f && %s>%f)",CUTPre,METCutSL,MassVisCutSL, tauPtCut, deltaRCutSL,plot,ptLep);
	  }else if(j==4){ 
	...
	if(save && i==0 && !SL) 
	  c1->SaveAs(name+"_MET"+met+"_lep1Pt"+lep1pt+"_tauPt"+taupt+"_btag"+btag+"_dR"+drcut+"_massSvfit"+masssvfitcut+"_massVis"+massviscut+"_FL1000.pdf");
	if(save && i==0 && SL)  
	  c1->SaveAs(name+"_MET"+met+"_lep1Pt"+lep1pt+"_tauPt"+taupt+"_btag"+btag+"_dR"+drcut+"_massSvfit"+masssvfitcut+"_massVis"+massviscut+"_SL1000.pdf");
      }
    }
  }
}
