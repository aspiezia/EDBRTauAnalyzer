{
  ...
  PLOT.push_back("met");            BIN.push_back(40); MIN.push_back(0);   MAX.push_back(1000); MAXY.push_back(10);   AXIS.push_back("met [GeV]");
  
  TH1F *METoptimization  = new TH1F("METoptimization","METoptimization",60,29,329);
  TH1F *SignalEfficiency = new TH1F("SignalEfficiency","SignalEfficiency",60,29,329);
  TH1F *BackgroundYield  = new TH1F("BackgroundYield","BackgroundYield",60,29,329);
  int KK=1;
  float MET = 25.;
  for(int i=0; i<60; i++){
    MET = MET + 5;
    ...
    for(int j=3; j<4; j++){
      char CUT [500]; 
      char openTree[500];
      if(j==0)      { sprintf(openTree, "demo/TreeEleMuo"); sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>%f)",MET);}
      else if(j==1) { sprintf(openTree, "demo/TreeMuoMuo"); sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>%f && MassVis>10)",MET);}
      else if(j==2) { sprintf(openTree, "demo/TreeEleEle"); sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>%f && MassVis>10)",MET);}
      else if(j==3) { sprintf(openTree, "demo/TreeMuoTau"); sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>%f && lepCorrPFIso<0.2)",MET);}
      else if(j==4) { sprintf(openTree, "demo/TreeEleTau"); sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>%f && lepCorrPFIso<0.1)",MET);}
    ...
    if(save && i==0) c1->SaveAs("MET_FullyleptonChannels_SR.png");
    ...
    float ZH1000Integral = 1618.73; //SEMI LEP
    //float ZH1000Integral = 789.459; //FULL LEP
    //cout<<endl;
    //cout<<MET<<endl;
    //cout<<"SIGNAL EFFICIENCY = "<<ZH1000->Integral()/1618.73<<endl;
    //cout<<"BACKGROUND YIELD  = "<<DY100->Integral()<<endl;
    //cout<<"PUNZI = "<<(ZH1000->Integral()/1618.73)/(1+sqrt(DY100->Integral()))<<endl;
    METoptimization->SetBinContent(KK, (ZH1000->Integral()/1618.73)/(1+sqrt(DY100->Integral())));
    SignalEfficiency->SetBinContent(KK, ZH1000->Integral()/1618.73);
    BackgroundYield->SetBinContent(KK, DY100->Integral());
    METoptimization->SetBinError(KK, 0.00001);
    SignalEfficiency->SetBinError(KK, 0.00001);
    BackgroundYield->SetBinError(KK, 0.00001);
    KK=KK+1;
  }
  METoptimization->Draw("E");
  METoptimization->SetMinimum(0.0);
  METoptimization->SetMaximum(0.2);
  METoptimization->SetMarkerStyle(21);
  METoptimization->SetLineColor(1);
  METoptimization->GetYaxis()->SetTitleSize(0.045);
  METoptimization->GetXaxis()->SetTitleSize(0.045);
  METoptimization->GetYaxis()->SetLabelSize(0.045);
  METoptimization->GetXaxis()->SetLabelSize(0.045);
  METoptimization->SetTitle("");
  METoptimization->GetXaxis()->SetTitle("MET threshold");
  METoptimization->GetYaxis()->SetTitle("#epsilon_{S}/1+#sqrt{B}");
  if(save) c1->SaveAs("MET_OPTIMIZATION_SR.png");
  BackgroundYield->Draw("E");
  BackgroundYield->SetMinimum(0.0);
  BackgroundYield->SetMarkerStyle(21);
  BackgroundYield->SetLineColor(1);
  BackgroundYield->GetYaxis()->SetTitleSize(0.045);
  BackgroundYield->GetXaxis()->SetTitleSize(0.045);
  BackgroundYield->GetYaxis()->SetLabelSize(0.045);
  BackgroundYield->GetXaxis()->SetLabelSize(0.045);
  BackgroundYield->SetTitle("");
  BackgroundYield->GetXaxis()->SetTitle("MET threshold");
  BackgroundYield->GetYaxis()->SetTitle("Background yield");
  if(save) c1->SaveAs("MET_BackgroundYield_SR.png");
  SignalEfficiency->Draw("E");
  SignalEfficiency->SetMinimum(0.0);
  SignalEfficiency->SetMarkerStyle(21);
  SignalEfficiency->SetLineColor(1);
  SignalEfficiency->GetYaxis()->SetTitleSize(0.045);
  SignalEfficiency->GetXaxis()->SetTitleSize(0.045);
  SignalEfficiency->GetYaxis()->SetLabelSize(0.045);
  SignalEfficiency->GetXaxis()->SetLabelSize(0.045);
  SignalEfficiency->SetTitle("");
  SignalEfficiency->GetXaxis()->SetTitle("MET threshold");
  SignalEfficiency->GetYaxis()->SetTitle("Signal Efficiency");
  if(save) c1->SaveAs("MET_SignalEfficiency_SR.png");
}
