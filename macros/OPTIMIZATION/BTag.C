{
  ...
  for(int k=0; k<2; k++){
    ...
    for(int i=0; i<14; i++){
      ...
      if(i==1)  sprintf(nbtag, "&& nbtagsL<1"); 
      if(i==2)  sprintf(nbtag, "&& nbtagsL<2"); 
      if(i==3)  sprintf(nbtag, "&& nbtagsL<3"); 
      if(i==4)  sprintf(nbtag, "&& nbtagsL<4"); 
      if(i==5)  sprintf(nbtag, "&& nbtagsL<5"); 
      if(i==6)  sprintf(nbtag, "&& nbtagsM<1"); 
      if(i==7)  sprintf(nbtag, "&& nbtagsM<2"); 
      if(i==8)  sprintf(nbtag, "&& nbtagsM<3"); 
      if(i==9)  sprintf(nbtag, "&& nbtagsM<4"); 
      if(i==10) sprintf(nbtag, "&& nbtagsT<1"); 
      if(i==11) sprintf(nbtag, "&& nbtagsT<2"); 
      if(i==12) sprintf(nbtag, "&& nbtagsT<3"); 
      if(i==13) sprintf(nbtag, "&& nbtagsT<4"); 
	...
	if(k==0){
	  if(j==0){ 
	    ...
	    sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>100 && XMassSVFit>250 && XMassSVFit<1750 %s)",nbtag);
	  }else if(j==1){ 
	    sprintf(openTree, "demo/TreeMuoMuo"); 
	    sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>100 && XMassSVFit>250 && XMassSVFit<1750 %s && MassVis>10)",nbtag);
	  }else if(j==2){ 
	    sprintf(openTree, "demo/TreeEleEle"); 
	    sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>100 && XMassSVFit>250 && XMassSVFit<1750 %s && MassVis>10)",nbtag);
	  }else if(j==3){ 
	    sprintf(openTree, "demo/TreeMuoTau");
	    sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>100 && XMassSVFit>250 && XMassSVFit<1750 %s && lepCorrPFIso<0.2)",nbtag);
	  }else if(j==4){ 
	    sprintf(openTree, "demo/TreeEleTau"); 
	    sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>100 && XMassSVFit>250 && XMassSVFit<1750 %s && lepCorrPFIso<0.1)",nbtag);
	  }
	}else{
	  if(j==0){ 
	    sprintf(openTree, "demo/TreeEleMuo"); 
	    sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>100 && XMassSVFit>1750 && XMassSVFit<3250 %s)",nbtag);
	  }else if(j==1){ 
	    sprintf(openTree, "demo/TreeMuoMuo"); 
	    sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>100 && XMassSVFit>1750 && XMassSVFit<3250 %s && MassVis>10)",nbtag);
	  }else if(j==2){ 
	    sprintf(openTree, "demo/TreeEleEle"); 
	    sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>100 && XMassSVFit>1750 && XMassSVFit<3250 %s && MassVis>10)",nbtag);
	  }else if(j==3){ 
	    sprintf(openTree, "demo/TreeMuoTau");
	    sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>100 && XMassSVFit>1750 && XMassSVFit<3250 %s && lepCorrPFIso<0.2)",nbtag);
	  }else if(j==4){ 
	    sprintf(openTree, "demo/TreeEleTau"); 
	    sprintf(CUT, "trigger*PUWeight*(sideband==0 && met>100 && XMassSVFit>1750 && XMassSVFit<3250 %s && lepCorrPFIso<0.1)",nbtag);
	  }
	}
	...
	c1->SetLogy();
      ...
      //if(save && i==0 && !SL && k==0) c1->SaveAs("nbtag_FL1000.png");
      //if(save && i==0 && !SL && k==1) c1->SaveAs("nbtag_FL2500.png");
      //if(save && i==0 && SL && k==0)  c1->SaveAs("nbtag_SL1000.png");
      //if(save && i==0 && SL && k==1)  c1->SaveAs("nbtag_SL2500.png");
  ...
  nbtagoptimization1->GetXaxis()->SetNdivisions(120);
  nbtagoptimization1->SetMinimum(0.0);
  nbtagoptimization1->SetMaximum(0.5);
  ...
  nbtagoptimization1->GetXaxis()->SetTitle("cut on btagged jet");
  nbtagoptimization1->GetYaxis()->SetTitle("#epsilon_{S}/1+#sqrt{B}");
  nbtagoptimization2->SetMarkerStyle(21);
  nbtagoptimization2->SetMarkerColor(2);
  nbtagoptimization2->SetLineColor(2);
  nbtagoptimization2->Draw("Esame");
  
  TLegend *pl = new TLegend(0.57,0.75,0.89,0.89);
  pl->SetTextSize(0.03); 
  pl->SetFillColor(0);
  TLegendEntry *ple = pl->AddEntry(nbtagoptimization1, "Signal - Mass = 1.0 TeV",  "LP");
  ple = pl->AddEntry(nbtagoptimization2, "Signal - Mass = 2.5 TeV",  "LP");
  pl->Draw();
  if(save && !SL) c1->SaveAs("nbtag_OPTIMIZATION_FL.png");
  if(save && SL)  c1->SaveAs("nbtag_OPTIMIZATION_SL.png");

  SignalEfficiency1->Draw("E");
  SignalEfficiency1->GetXaxis()->SetNdivisions(120);
  ...
  SignalEfficiency1->GetXaxis()->SetTitle("cut on btagged jet");
  ...
  pl->Draw();
  if(save && !SL) c1->SaveAs("nbtag_SignalEfficiency_FL.png");
  if(save && SL)  c1->SaveAs("nbtag_SignalEfficiency_SL.png");



  BackgroundYield1->Draw("E");
  BackgroundYield1->GetXaxis()->SetNdivisions(120);
  ...
  BackgroundYield1->GetXaxis()->SetTitle("cut on btagged jet");
  ...
  pl->Draw();
  if(save && !SL) c1->SaveAs("nbtag_BackgroundYield_FL.png");
  if(save && SL)  c1->SaveAs("nbtag_BackgroundYield_SL.png");
}
