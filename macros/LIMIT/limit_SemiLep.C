...
void limit_SemiLep(bool only_leptonTAG=false, bool save_plots = false, float min=0.006, float max=900., TString plot_name="Limit_prova.pdf") {
  setTDRStyle();
  TCanvas* c1  = new TCanvas("c1","c1",0,0,800,600); //c1->SetLogy();
  c1->SetLogy();
  save_plots = true;
  plot_name="limit_SemiLep.png";

  Double_t XMass[val]    = {1000, 1500, 2000, 2500};
	
  //LIMIT - Breit-Wigner
  Double_t Limit_Obs[val]     = {0,0,0,0};
  Double_t Limit_Exp_m2s[val] = {2.9682 , 7.6243 , 32.4566 , 83.5495 };
  Double_t Limit_Exp_m1s[val] = {4.1161 , 10.6905, 45.8438 , 118.5567};
  Double_t Limit_Exp[val]     = {6.1279 , 16.0645, 70.1172 , 182.0312};
  Double_t Limit_Exp_p1s[val] = {9.1812 , 25.6695, 113.1587, 288.6921};
  Double_t Limit_Exp_p2s[val] = {13.0584, 42.5113, 179.6031, 443.8708};

  //LIMIT - GaussXLandau
  Double_t Limit_Obs2[val]     = {0,0,0,0};
  Double_t Limit_Exp_m2s2[val] = {2.5653 , 9.5942 , 30.4874 , 70.2347 };
  Double_t Limit_Exp_m1s2[val] = {3.6234 , 13.3695, 43.5860 , 101.0882};
  Double_t Limit_Exp2[val]     = {5.5420 , 20.2148, 68.1641 , 157.0312};
  Double_t Limit_Exp_p1s2[val] = {8.7451 , 31.8985, 115.4407, 255.9287};
  Double_t Limit_Exp_p2s2[val] = {12.8621, 50.1499, 197.9498, 406.5341};
  ...
  legendr->AddEntry(band_1s, "Expected #pm 1#sigma - Breit-Wigner","flP");
  legendr->AddEntry(band_2s, "Expected #pm 2#sigma - Breit-Wigner","flP");
